...
ightarrow$ **Create repository**)

#### A. docker-hosted (Push 전용)
- **Recipe**: `docker (hosted)`
- **Name**: `docker-hosted`
- **HTTPS**: Check $
ightarrow$ Port: `8082` 입력
...
> **시연 검증 포인트**
> 1. **Nexus Web UI** $
ightarrow$ **Repositories** $
ightarrow$ **docker-proxy** $
ightarrow$ **Browse** 이동 시 `nginx` 이미지가 자동 Caching된 것을 확인합니다.
